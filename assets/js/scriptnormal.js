...
function CalculoFinal() {
    ...
    if (hijosmediacionok && gastostotalesok && ingresosmadreok && ingresospadreok) {
        ...
        document.getElementById("resultadostexto").innerHTML = (`
            ...
            Gastos totales mensuales: $${gastostotales.value.replace(regexpuntos,".")}<br>
            Ingreso mensual promedio del padre: $${ingresospadre.value.replace(regexpuntos,".")} (${(Math.round(porcentajepadre*100))}% del total)<br>
            Ingreso mensual promedio de la madre: $${ingresosmadre.value.replace(regexpuntos,".")} (${Math.round(((1-porcentajepadre)*100))}% del total)<br>
            <br>
            <b>RESULTADOS:</b><br>
            <br>
            PENSIÓN QUE DEBE PAGAR EL PADRE: $${parseInt(pensionapagar).toString().replace(regexpuntos,".")}<br>
            <br>
            <br>
        `);
    } else {
